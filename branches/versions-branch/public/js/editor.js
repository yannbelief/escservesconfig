var EscEditor = function() {
    return {
		...
		createTableForKeyValues : function(env, keyValues) {
        	...
        	$.each(keyValues, function(i, item) {
            	...
            	table += ("</td>");
				table += ("<td class='edittablebutton'>");
            	if ( !item.isEncrypted && item.isOverridden && env != 'default') {
            ...
            $.ajax({
				...
				complete: function(XMLHttpRequest, textStatus) {
			       if ( textStatus == "success" ) {
                    	...
                    	var table = EscEditor.createTableForKeyValues(env, keyValues);
						$('#editor').append(table);
